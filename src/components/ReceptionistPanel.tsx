                ...
                >
                  <div className="space-y-1 min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500">
                      Total Paid: {ethers.formatEther(room.totalPrice || room.pricePerNight)} ETH
                      {room.checkInTime > 0n && ` · ${new Date(Number(room.checkInTime) * 1000).toLocaleDateString()} to ${new Date(Number(room.checkOutTime) * 1000).toLocaleDateString()}`}
                    </p>
                  </div>
                  <button
                    disabled={txPending}
                    onClick={() => handle(room.id, confirmCheckIn)}
                    className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 text-white text-xs font-bold rounded-xl transition active:scale-95 whitespace-nowrap cursor-pointer"
                  >
